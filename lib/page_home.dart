...
//   Mode(this.name, this.description, this.category, this.icon, this.mode);
  ...
  Future<void> _pickRandomCountry() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to load country: ${response.statusCode}')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Error fetching country: $e')));
    }
  }
  Future<void> _pickRandomFlag() async {
    List<String> ccn3List = [
      "004",
      "008",
      "012",
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to load flag: ${response.statusCode}')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Error fetching flag: $e')));
    }
  }
